...
The server resolves givers in this order (`server/game/systems/quests.ts`):
1. Match a live mob whose `spawn_id` equals the giver string.
2. If no mob in the zone has that spawn_id, fall back to matching `template_id`.

**Default to template id as the giver.** Zone files can be regenerated by world-growth
pipelines, which may strip `spawn_id` fields from spawn entries. A giver set to a
template id is resilient to this; a giver set to a spawn_id silently breaks if the
spawn_id disappears.

Only use spawn_id when the same template exists in multiple zones and the quest must
be offered only by the specific named instance — for example, a named quest NPC who
shares a template with generic guards.

To assign a spawn_id when it is genuinely needed, add it to the mob's spawn entry
in the zone:
```jsonc
