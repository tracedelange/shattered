...
check('unknown feature id flagged', validateReferences(base({
  file_ops: [{ op: 'append_features', zone_id: someZone, features: ['no_such_feature'] }],
}), defs).length === 1);

check('feature portal to spec-created zone passes', validateReferences(base({
  new_zones: [{ id: 'test_cellar', biome: 'dungeon', name: 'T', parent_zone: someZone, lore_summary: 's' }],
  file_ops: [{ op: 'append_features', zone_id: someZone, features: [{ id: 'cellar_entrance', portal_to: 'test_cellar' }] }],
}), defs).length === 0);

check('feature portal to unknown zone flagged', validateReferences(base({
  file_ops: [{ op: 'append_features', zone_id: someZone, features: [{ id: 'cellar_entrance', portal_to: 'no_such_zone' }] }],
}), defs).length === 1);

check('portal_to on a feature operator flagged', validateReferences(base({
  file_ops: [{ op: 'append_features', zone_id: someZone, features: [{ id: 'fountain', portal_to: someZone }] }],
}), defs).length === 1);
...
console.log('\ndevelopment metrics');
{
  ...
  const village = m.zones.find((z) => z.id === 'village_41_41');
  check('village has spawns counted', (village?.total_spawns ?? 0) > 0);
  check('village has cellar sub-zone', village?.subzones.includes('zone_41_41_sewer') === true);
  ...
  check('frontier signals reference real zones',
    m.signals.frontier.length > 0 && m.signals.frontier.every((f) => f.zone in defs.zones));
  check('connectivity graph computed', m.graph.connected_components >= 1);
  check('disconnected list consistent with component count',
    (m.graph.connected_components === 1) === (m.graph.disconnected_zones.length === 0));
  const cellar = m.zones.find((z) => z.id === 'zone_41_41_sewer');
  check('cellar records its parent', cellar?.parent_zone === 'village_41_41');
  const trimmed = trimMetricsForDisk(m);
  check('disk view trims pristine rows', trimmed.zones.length < m.zones.length);
  check('disk view keeps developed rows', trimmed.zones.some((z) => z.id === 'village_41_41'));
}
...
console.log('\nanti-repetition digest');
{
  const history = [
    ...
    '  - { opportunity_id: opp_003, type: quest_add, target_zone: village_41_41, implemented_at: x, files_written: [], files_modified: [], notes: n }',
  ].join('\n');
  const digest = formatRecentWorkDigest(history);
  check('digest counts types', digest.includes('mob_populate ×2'));
  check('digest lists targets', digest.includes('village_41_41'));
