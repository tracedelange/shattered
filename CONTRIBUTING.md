...
Two workflows, both originally generated by `firebase init hosting:github` and
then edited to add the build steps the CLI omits:
...
- `firebase-hosting-merge.yml` — builds and deploys to the live channel of the
  ...
  those are pushes to `main`), and on demand from the Actions tab.
- `firebase-hosting-pull-request.yml` — builds and deploys each PR to its own
  preview channel, commenting the URL on the PR. Skipped for PRs from forks,
  which can't read the service account secret.
...
checkout-then-deploy workflows would have published an empty directory over the
live site. **Re-running `firebase init hosting:github` overwrites both files** —
...
name is the CLI's convention (`FIREBASE_SERVICE_ACCOUNT_<PROJECT>`); both
workflows read exactly that.
