...
//
// Lives in shared/ so server (collision, spawn) and client (render) compute
// identical terrain from the same seed — the determinism contract (R8.7/R8.8).
// server/game/mapgen/rng.ts re-exports these for existing importers.
...
// Box-Muller transform: draws from a normal distribution using a [0,1) RNG.
// Deterministic given a deterministic `rng` (e.g. mulberry32).
export function gaussianSample(rng: () => number, mean: number, sd: number): number {
